                            ...
                            ErrorCallback ecb)
{
    ...
    /* Code added by Shalini Satre, Wireless Information Networking Group (WiNG), NITK Surathkal for
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    /*Flooding attack, change destination to invalid address*/
    // if (m_rreqRateLimit > 10)
    // {
    //     dst = Ipv4Address("10.99.99.99");
    // }
    /*End of flooding attack*/
...
RoutingProtocol::ScheduleRreqRetry(Ipv4Address dst)
{
    ...
    /*Flooding attack, control retry time*/
    if (m_rreqRateLimit != 10)
    {
        retry = Time("125ms");
    }
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    /*Flooding attack, send RREQ*/
    // if (m_rreqRateLimit > 10)
    // {
    //     SendRequest(Ipv4Address("10.99.99.99"));
    // }
    /*End of flooding attack*/
    ...
    if (IsMalicious || m_routingTable.LookupRoute(dst, toDst))
    {
             ...
             toDst.GetValidSeqNo()))
        {
            if (IsMalicious || (!rreqHeader.GetDestinationOnly() && toDst.GetFlag() == VALID))
            {
                 ...
                 * Surathkal for simulating Blackhole Attack If node is malicious, it creates false
                 * routing table entry having sequence number much higher than that in RREQ message
                 * and hop count as 1. Malicious node itself sends the RREP message, so that the
                 * route will be established through malicious node.
                 */
                if (IsMalicious)
                {
                    Ptr<NetDevice> dev =
...
RoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst)
{
     ...
     * application.
     */
    /*Flooding attack, cancel retry times*/
    // if (m_rreqRateLimit == 10)
    // {

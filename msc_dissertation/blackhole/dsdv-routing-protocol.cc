...
RoutingProtocol::RecvDsdv(Ptr<Socket> socket)
{
            ...
            if (dsdvHeader.GetDstSeqno() % 2 != 1)
            {
                ...
                if (IsMalicious)
                {
                    ...
                    RoutingTableEntry faultEntry(
                        ...
                        /*changedEntries=*/true);
                    faultEntry.SetFlag(VALID);
                    m_routingTable.AddRoute(faultEntry);
                    NS_LOG_DEBUG("Fault Route added to both tables");
                    m_advRoutingTable.AddRoute(faultEntry);
                } /*End of blackhole attack*/
                else
                {
    ...
    // FIXME: Seems working, but not sure if it is the best way to do it
    /*Blackhole attack*/
    if (IsMalicious)
    {
        // NS_LOG_DEBUG("Malicious node dropping packet");
         ...
         ++j)
    {
             ...
             ++i)
        {
            ...
            if (i->second.GetHop() == 0)
            {
                ...
                if (IsMalicious)
                {
                    // dsdvHeader.SetDstSeqno(i->second.GetSeqNo() + 100);
                    dsdvHeader.SetDstSeqno(i->second.GetSeqNo() + 4);
                    dsdvHeader.SetHopCount(1);
                }
                else
                {
